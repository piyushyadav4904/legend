...
public class LinenmentsActivity extends AppCompatActivity{
private ProgressBar progressBar;
    private ArrayList<LinenmentsModel> list;
    private LinementsAdapter adapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_linenments);
        list=new ArrayList<LinenmentsModel>();
        progressBar = findViewById(R.id.progress_cart);
        RecyclerView recyclerView = findViewById(R.id.recycler_cart);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        adapter = new LinementsAdapter(this, list);
            ...
            public void onResponse(JSONArray response) {
                for(int i=0;i<=response.length();i++){
                    try {
                        ...
                        LinenmentsModel dto = new LinenmentsModel();
